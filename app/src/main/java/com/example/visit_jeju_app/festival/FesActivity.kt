...
import com.example.visit_jeju_app.databinding.ActivityFesBinding
import com.example.visit_jeju_app.festival.adapter.FesAdapter
...
class FesActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.pageChange.setOnClickListener {
            val intent = Intent(this@FesActivity, FesRegionNmActivity::class.java)
            startActivity(intent)
        }
        ...
        toggle =
            ActionBarDrawerToggle(this@FesActivity, binding.drawerLayout, R.string.open, R.string.close)
        ...
        bottomNavigationView.setOnNavigationItemSelectedListener { item: MenuItem ->
            when (item.itemId) {
                R.id.home -> {
                    // 홈 아이템 클릭 처리
                    val intent = Intent(this@FesActivity, MainActivity::class.java)
                    startActivity(intent)
                    true
                }
                R.id.chat -> {
                    val intent = Intent(this@FesActivity, GptActivity::class.java)
                    startActivity(intent)
                    true
                }
                R.id.youtube -> {
    ...
    private fun getFesListWithinRadius(coords: String) {
        ...
        fesListCall.enqueue(object : Callback<List<FesList>> {
            override fun onResponse(
                ...
                val fesistSpotsWithinRadius = fesList?.mapNotNull { spot ->
                ...
                val layoutManager = LinearLayoutManager(this@FesActivity)

                binding.recyclerView.layoutManager = layoutManager

                binding.recyclerView.adapter =
                    FesAdapter(this@FesActivity,fesistSpotsWithinRadius)
    ...
    override fun onCreateOptionsMenu(menu: Menu?): Boolean {
        ...
        searchView.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
            ...
            override fun onQueryTextSubmit(query: String?): Boolean {
                ...
                Toast.makeText(this@FesActivity,"검색어가 전송됨 : ${query}", Toast.LENGTH_SHORT).show()
                return true
            }
        })
